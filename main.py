...
def track_user(user_id: int):
    now = int(time.time())
    redis_cmd(["SADD", "users:total", user_id])
    redis_cmd(["HSET", "users:active", user_id, now])

def get_stats():
    now = int(time.time())
    one_hour_ago = now - 3600

    total = redis_cmd(["SCARD", "users:total"]).get("result", 0)
    active_raw = redis_cmd(["HGETALL", "users:active"]).get("result", [])

    active = 0
    for i in range(0, len(active_raw), 2):
        last_seen = int(active_raw[i + 1])
...
user_languages = {}
active_users = set()  # untouched, still used for broadcast
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            try:
                ...
                await bot.send_message(chat_id=chat_id, text=MESSAGES["timeout"][lang])
                return

            file_size = os.path.getsize(filename) / (1024 * 1024)
            if file_size > 50:
                await bot.send_message(chat_id=chat_id, text=f"{emoji} {MESSAGES['download_too_big'][lang]}")
                return
            ...
            await bot.send_message(chat_id=chat_id, text=f"{emoji} {MESSAGES['download_done'][lang]}")
